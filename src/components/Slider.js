...
import Slide_1 from '../img/slide-3.png'
...
class Slider extends React.Component {
		constructor(props) {
			...
			this.state = {
				slides : [
					{
						eachLide : `url(${Slide_1})`
					},
					{
						eachLide : `url(${Slide_2})`
					},
					{
						eachLide : `url(${Slide_3})`
					},
					{
						eachLide : `url(${Slide_4})`
					}
				],
				autoPlay: false,
				active: 0,
		...
		renderSlides() {
			...
			return this.state.slides.map((item, index) => (
					<div 
						...
						key = {index}
						style = {{backgroundImage: item.eachLide, width: transition}}>
					</div>
				))
		}
		renderDots() {
